...
public class TemaControlador {
	...
	@PostMapping
	public ResponseEntity<Tema> addTema(@Valid @RequestBody Tema novoTema) {
		return ResponseEntity.status(201).body(repositorio.save(novoTema));
	}
	
	@DeleteMapping("/deleta/{idTema}")
	public void deletaTema(@PathVariable (value = "idTema") Long idTema) {
		repositorio.deleteById(idTema);
	}
	
	@PutMapping
	public ResponseEntity<Tema> atualizaTema(@Valid @RequestBody Tema atualizaTema) {
		return ResponseEntity.ok().body(repositorio.save(atualizaTema));
	}
}
